...
{
    $orderId = $_GET['id'];
    $sql = "SELECT * FROM per_orders WHERE id = $orderId";
    $result = mysqli_query($connect, $sql);
    $order = mysqli_fetch_assoc($result);

    // Fetch order items
    $sqlItems = "SELECT * FROM per_order_items WHERE order_id = $orderId";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
    ...
    $updateSql = "UPDATE per_orders SET order_status = '$order_status' WHERE id = $orderId";
    
    if (mysqli_query($connect, $updateSql)) 
    {     
        echo "<script>alert('Order updated successfully');</script>";
        echo "<script>window.location.href = 'per_ordering_view.php?id=$orderId';</script>";
    } else 
    {
